...
axios.post(`${directoryServerUrl}/register`, { serverName: "Server2", address: `http://localhost:${port}` });
...
app.post("/", async (req, res) => {
  if (req.body && req.body.message === "pong") {
    ...
    const response = await axios.post(`${directoryServerUrl}/getAddress`, { serverName: "Server1" });
    const server1Url = response.data.address;

    // Attendre une demi-seconde avant d'envoyer la requête "ping" vers le serveur 1
    setTimeout(async () => {
      await axios.post(`${server1Url}`, { message: "ping" });
    }, 2000);
